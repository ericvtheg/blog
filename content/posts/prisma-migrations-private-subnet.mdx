...
Most of us who have had a hobby project using Prisma love the amazing developer experience of generated types and easy schema migrations.
Eventually we get to the point where we need to actually deploy and host our code so users can use interact with our software (that is the goal after all).
This mean we need to do schema updates and migrations in our production environment without downtime.
Now as good software engineers we know to keep our database safe from the outside world by keeping it in a private subnet.
But this means that we must deal with the issue of somehow running our Prisma migrations within the private subnet as well.
...
```
{
  ...
  "scripts": {
    "start:prod": "prisma:migrate && node dist/src/main",
