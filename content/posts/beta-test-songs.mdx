...
Create something with as little work as possible that still helps people (i.e. I want to build an MVP).
...
The primary reason I opted for SQL was for extendability. Features such as multiple reviews per song and select a random song would work better in a relational database.
...
- Prevented users viewing their own track submissions despite there being no login / accounts
- Using Mixpanel to track page landings and first time visits

## Mistakes

Recall that my primary goal with this project was to help users. The mistakes I made around that goal helped me learn.

## Over Engineering

A mistake that stood out was the one I was originally trying to avoid: over engineering.
...
The site's SEO is not good. I've submitted the site to Google's Seach Console and the site still does not show up on Google.

This mistake may have to do with the former: I accidentally deployed a version of my site that had an api route to localhost. In other words I had localhost hardcoded.
When I visited the site through my browser Google **really** did not like this and actually blacklisted my site and warned users not to visit.
I had to submit a request for them to review that the site is now safe. This was a really bad error and one I will not make again.
...
2. Just use Vercel. It's all you need for spinning up a web app quickly.
   ...
   -- The best part, it even comes with a backend where you can store secrets and do any sort of server application logic required.
