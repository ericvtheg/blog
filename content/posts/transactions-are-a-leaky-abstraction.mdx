...
description: Transactions empower application developers, but we must understand what occurrs under the hood.
date: "2023-08-15"
---
...
to let us design our systems as if just a single process is modifying state serially. [1]
...
If this was the case, we wouldn't need to worry about
exceptional circumstances where our data was modified in ways we didn't anticipate.
...
Each level has potentially detrimental _gotchas_ that we must be aware of.
In order to build robust systems we must understand how transactions
work and what level of isolation our database uses.
...
data consistency but worse performance. [1]

Understanding how our databases writes and reads is crucial for mitigating the potential
of race conditions between our concurrent transaction. To add even more complexity, there
is no standard for isolation levels across databases. For example snapshot isolation in
PostgreSQL is called repetable read, while in Oracle it is called serializable. [2]
...
firemware at the bottom. For the purpose of this article we will focus on race conditions
...
> 2. When writing to the database, you will overwrite data that has been committed (no dirty writes).

<cite>&mdash; Martin Kleppmann</cite> [3]
...
that can be avoided using repeatable committed.
...
data that is being read/written will stay the same from the perspective of the transaction. [4]
...
An excellent example of a race condition that is avoided through the use of snapshot isolation is when
...
there will be an inconsistency in the backup data where $100 went magically went missing.
...
Write skew is a easy to miss example of a race condition. Write skew is caused by phantoms, in other words,
data that does not exist. Write skew occurs when database operations depend on the abscence of data.
...
As you can see we're checking that there is less than 20 attendees scheduled for class X.
...
This is a sneaky race conditon, but the serialization isolation level solves this right? For most implementations, no.
...
1. Literally executing each transaction one at a time [6]
2. Using predicate locks [8]

Predicate locks work by locking a whole range of rows that are accessed in the query, whether they exist or not.
